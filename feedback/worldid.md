...
**What we built with it:** Nautica is a citizen-science game where you sign in with
World ID, complete photo quests to earn XP, and at Level 5 unlock **partner-funded
paid quests that pay USDC on Base**. World ID is load-bearing, not decoration:
**Selfie Check** is our one-human sign-in and the **sybil gate** that protects the XP
leaderboard and the shared USDC payout pool — without proof-of-humanity, both are
farmed to zero by multi-accounting. Identity Check / Orb sit above it as stronger
tiers for the paid path.
...
feedback**, followed by the value questions from the track.
...
The primitives are strong and the happy path is short once you know the shapes; we
...
`@worldcoin/idkit-core/signing`, key stays server-side, short TTL — is a genuinely
good design, and the controlled `IDKitRequestWidget` (`open` / `onSuccess`) dropped
...
### Ease of integration — where the SDK/docs helped, and where they got in the way
...
Got in the way (each cost real time, all verified against the live verifier):
...
**Developer suggestions:** document the v3-vs-v4 proof shape (`string` vs `string[]`)
and identifier vocabulary (`selfie` vs `face`) side by side; give `allow_legacy_proofs`
guidance per preset; fix the 404 quick-start; ship one copy-pasteable full-stack
example (register RP, `signRequest`, open widget, v4 verify).
...
**User suggestions:** provide recommended, outcome-oriented microcopy for the selfie
step (why + what happens with my face data + it's one-time); and clearer
distinct guidance for the camera-permission-denied and credential-unavailable states.
...
KYC-ing each one" possible for us; remove it and the reward economy is farmed to zero.
...
added friction. Our thesis is that Selfie Check optimizes onboarding conversion while
POH optimizes payout integrity, and the product should let each do what it's best at.
